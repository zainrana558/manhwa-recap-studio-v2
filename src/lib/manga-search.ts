...
export async function searchAllManga(
  ...
  const [mhRes, ffRes, wtRes, malRes, alRes] = await Promise.allSettled([
  ...
  const deduped: MangadexManga[] = [];
  for (const m of [...mangahere, ...fanfox, ...webtoons, ...mal, ...anilist]) {
  ...
  // Sort: scrapeable sources first (mangahere, fanfox, webtoons), then metadata.
...
export async function searchSingleSource(
  query: string,
  source: "mangahere" | "fanfox" | "webtoons" | "mal" | "anilist",
  limit = 12
